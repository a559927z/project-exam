    ...
    function ga(b) {
        ...
        i = {
            ...
            nextTick: g.nextTick, onError: w, configure: function (a) {
                ...
                if (typeof j.urlArgs === 'string') {
                    return d + ((-1 === d.indexOf("?") ? "?" : "&") + j.urlArgs)
                } else if (typeof j.urlArgs === 'function') {
                    var urlArgs;
                    try {
                        urlArgs = j.urlArgs(a.join("/"), b, c, d)
                    } catch (e) {
                        urlArgs = ""
                    }
                    return d + ((-1 === d.indexOf("?") ? "?" : "&") + urlArgs)
                }
...
// 默认前面路径 ：assets/js/
    var paths = {
            ...
            "layer": "../layer-v3.0.3/layer/layer", // 弹出层
            "spinJs": "../spin-2.1.0/jquery.spin.merge", // 罩
            "dataTable": "../datatables/js/jquery.dataTables", // 数据表
            "datatables.net": "../datatables/js/dataTables.bootstrap", // 用上datatables用上bootstrap3后这个必须要用datatables.net命名
            'dataTablePush': '../datatables/dataTablePush',//dataTable封装
        },
        ...
        baseUrl = function () {
            ...
            return getAbsoluteSrc(script).replace("require-min.js", '');
        }(),
